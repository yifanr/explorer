...
def make_step(
    ...
    loss_value, grads = update_GMM(model, x, key)
...
for step, (x, y) in zip(range(STEPS), infinite_trainloader()):
    ...
    if (step % 1000) == 0 or (step == STEPS - 1):
        ...
        print(jnp.min(jnp.linalg.eigh(model.covariances)[0]))
