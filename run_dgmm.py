...
from network import DGMMVAE
from gradients import update_DGMM
import optax
...
model = DGMMVAE(28*28, 2, NUM_CLUSTERS, subkey)
# model = AE(28*28, 8, jax.random.PRNGKey(42))

filter = jtu.tree_map(lambda _: False, model)
filter = eqx.tree_at(
    lambda tree: (tree.means, tree.sizes, tree.variances),
    filter,
...
def make_step(
    ...
    (loss_value, (reconstruction_loss, size_loss, mean_loss, variance_loss)), grads = update_DGMM(model, x, key)
...
for step, (x, y) in zip(range(STEPS), infinite_trainloader()):
    ...
    if (step % 500) == 0 or (step == STEPS - 1):
        ...
        x = jnp.reshape(x, (x.shape[0], 28*28))
        (loss_value, (reconstruction_loss, size_loss, mean_loss, variance_loss)), grads = update_DGMM(model, x, subkey)
