...
def unsupervised_loss(
    ...
    encoding = eqx.filter_vmap(network.encode)(x, keys)
    decoding = eqx.filter_vmap(network.decode)(encoding)
    total_loss = jnp.sum((decoding - x) ** 2)
    return total_loss, encoding
    ...
    for step, (x, y) in zip(range(steps), infinite_trainloader()):
        ...
        if (step % print_every) == 0 or (step == steps - 1):
            ...
            for i in range(10):
                ...
                plt.show()
                encoding = network.encode(x[i], subkey)
                reconstruction = network.decode(encoding)
...
unsupervised_network = SimpleNetwork(layers)
